...
import com.digitald4.nbastats.distributed.hadoop.ProcessFanDuel;
...
public class FantasyProcessor {
	...
	public static void main(String[] args) throws Exception {
		...
		String logFile = "input/fanduel/process/groupA.csv";
		...
		Broadcast<String[]>  projectionMethodsBroadcast = javaCtx.broadcast(br.readLine().split(","));
		...
		br = new BufferedReader(new FileReader(DATA_PATH + "groupB.csv"));
		while ((line = br.readLine()) != null) {
		...
		br.close();
		bGroups.sort(Comparator.comparing(PlayerGroup::getCost));
		Broadcast<List<PlayerGroup>> bGroupsBroadcast = javaCtx.broadcast(bGroups);

		List<PlayerGroup> cGroups = new ArrayList<>(1000);
		br = new BufferedReader(new FileReader(DATA_PATH + "groupC.csv"));
		while ((line = br.readLine()) != null) {
		...
		br.close();
		Broadcast<List<PlayerGroup>> cGroupsBroadcast = javaCtx.broadcast(cGroups);
		...
		Map<String, TopLineUps> lineups = aGroupsRDD
				.map(aGroup -> {
					...
					List<PlayerGroup> bGroups_ = bGroupsBroadcast.getValue();
					cGroupsBroadcast.getValue().forEach(cGroup -> {
						for (PlayerGroup bGroup : bGroups_) {
				...
				.reduce((topLineupsMapA, topLineupsMapB) -> {
					for (String method : projectionMethodsBroadcast.getValue()) {
						TopLineUps topLineupsA = topLineupsMapA.get(method);
						TopLineUps topLineupsB = topLineupsMapB.get(method);
						if (topLineupsA == null) {
							topLineupsMapA.put(method, topLineupsB);
						} else {
							topLineupsA.addSorted(topLineupsB);
						}
					}
					return topLineupsMapA;
				});
		FileWriter fileWriter = new FileWriter(String.format(ProcessFanDuel.OUTPUT_PATH, dateStr));
