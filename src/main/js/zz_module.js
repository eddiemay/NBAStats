com.digitald4.nbastats.module = angular.module('nbastats', ['DD4Common', 'ngRoute', 'ui.date'])
    ...
    .controller('NBAStatsCtrl', function($location, apiConnector, globalData, $filter) {
      if ($location.host() == 'localhost') apiConnector.baseUrl = 'https://fantasy-predictor.appspot.com/';
      ...
      globalData.season = globalData.date.getFullYear();
      if (globalData.date.getMonth() > 9) {
        globalData.season++;
      }
      ...
      globalData.getSeason = function() {
        return (globalData.season - 1) + '-' + (globalData.season % 100);
      }
    ...
    .service('playerDayService', function(apiConnector) {
      ...
      service.processStats = function(date, onSuccess, onError) {
        this.sendRequest({action: 'processStats', method: 'POST', data: {date: date}}, onSuccess, onError);
      }
      return service;
    })
